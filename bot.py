...
class Bot:
    ...
    def no_answer(self, question: str):
        ...
        if rnd_val > 100:
            return random.choice(const.DONT_KNOW_PHRASES)
        ...
        for phrase in self.phrases:
            ...
            if any(s.count(phrase_0) > 0 and len(s) > 1 for s in question_words) and len(line_list) > 1:
                return random.choice(line_list[1:])
        
        return services.get_first(random.choice(self.phrases))
    ...
    def find_answer(self, question):
        ...
        if res["answers"]:
            return random.choice(res["answers"])
        return None
...
def tests():
    phrase = 'кто же ты такой'
    print(Bot(True).find_answer(phrase))
...
- попробовать подбирать ответы по числу вхождения: букв, слогов
    + проверил по числу букв: пришлось ограничить минимум
- сохранять ответ 1 раз, в строке сохранять номер строки с ответом через ';'
"""
